...
const Menu = () => {
  const { data: menu, loading, error } = useFetch(`${BASE_URL}/services`);
  console.log("hihiihi", menu);
...
export const AllMenuData = ({ item }) => {
  const { _id, name, subservices, imageUrl, price, duration } = item;
  ...
  return (
    <>
      <td>
        <img className="img-fluid rounded-2 menu-item-img" src={imageUrl} />
        {name}
      </td>
      <td>
        <ul>
